...
--test_run:switch('default')
--box.cfg{replication_synchro_quorum=BROKEN_QUORUM, replication_synchro_timeout=30}
--_ = box.schema.space.create('sync', {is_sync=true, engine=engine})
--_ = box.space.sync:create_index('pk')
...
new_s = s:update({{'=', 6, {is_sync=false}}})
...
-- Option replication_connect_timeout works as expected with sync cluster.
-- TODO

-- Option replication_sync_lag works as expected with sync cluster.
-- TODO

-- Option replication_sync_timeout works as expected with sync cluster.
-- TODO

-- Option replication_timeout works as expected with sync cluster.
-- TODO
...
-- [RFC, Recovery and failover] In case there's not enough replicas to set up
-- a quorum the cluster can be switched into a read-only mode.
-- TODO
...
box.space.sync:select{} -- 1
box.snapshot()
-- TODO: Make sure data is in snapshot.
box.space.sync:select{} -- 1
-- Testcase cleanup.
box.space.sync:drop()
...
-- [RFC, quorum commit] check behaviour with no answer from a replica during
-- write, expected to set failure answer.
...
box.error.injection.set('ERRINJ_WAL_IO', false)
box.space.sync:select{} -- 1
-- TODO: check replica disconnect
-- Testcase cleanup.
test_run:switch('default')
box.space.sync:drop()

-- ERRINJ_WAL_IO, ERRINJ_BOOL, {.bparam = false}
-- ERRINJ_WAL_SYNC, ERRINJ_BOOL, {.bparam = false}
-- ERRINJ_WAL_ROTATE, ERRINJ_BOOL, {.bparam = false}
-- ERRINJ_WAL_WRITE, ERRINJ_BOOL, {.bparam = false}
-- ERRINJ_WAL_WRITE_PARTIAL, ERRINJ_INT, {.iparam = -1}
-- ERRINJ_WAL_WRITE_DISK, ERRINJ_BOOL, {.bparam = false}
-- ERRINJ_WAL_WRITE_EOF, ERRINJ_BOOL, {.bparam = false}
-- ERRINJ_WAL_DELAY, ERRINJ_BOOL, {.bparam = false}
